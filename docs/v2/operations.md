...
**No command, environment variable or deployment step changed.** The console is still
one static bundle served for every tenant, still resolves the tenant from the
subdomain, and still runs against MSW until `VITE_USE_MOCK=0` points it at a real API.
