...
/** The newest month with bills, which is what the month reports are worth running on. */
...
describe('missingReportParams (the shared rule)', () => {
  it('asks each report for exactly what it needs', () => {
    ...
    // Zero is a real answer — "no leaf at all this month" — so it must not read as missing.
