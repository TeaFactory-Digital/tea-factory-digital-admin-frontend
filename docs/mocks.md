...
| `422 note-required` | AC-06, on **both** approve and reject, and on suspend and reveal |
...
| --- | --- | --- | --- |
| `clerk@galabodatea.lk` | `clerk` | no | Works the queue |
| `manager@galabodatea.lk` | `manager` | **yes** — code `123456` | Approves; exercises the MFA step |

Password for both: `demo1234`. Both are **printed on the sign-in screen** while
`VITE_USE_MOCK` is on — deliberate, because a demo credential that has to be looked
up in a source file gets pasted into a chat thread and outlives the demo. The block
cannot render in a production build.

Two identities, because AC-10 ("no console user can approve a record they created")
cannot be demonstrated with one.
