...
| §20 M16 Reports | One report id, not four |
...
200 {
  ...
  "flags": {
    ...
    "enableManure": true, "enableTeaPackets": true, "enableInquiry": true,
    "enableNews": true, "enablePushNotifications": true, "enablePromoBanner": true,
    "enableOnboarding": true, "enableBiometricLogin": true,
    "enableDarkModeToggle": true, "enableProfileTab": true, "enableAutoLock": true
  },
...
200 {
  ...
  "app": {
    "suppliersWithApp": 64, "totalSuppliers": 82,
    "devicesRegistered": 67, "appRequestShare": 0.71
  },
  "content": {
    "articlesWithGaps": 2, "bannersLive": 1,
    "bannersExpired": 1, "staticPagesUnwritten": 1
  },
  "adoptionTrend": [
    { "monthKey": "2025-09", "appShare": 0.32 },
    { "monthKey": "2025-10", "appShare": null }
  ],
  "alerts": [
...
- **`app` is the console's headline**, and §19.3 is why: app adoption and channel
  shift are *"the two KPIs that justify the project"*, and nothing else in the
  factory can answer them. `suppliersWithApp` counts suppliers with at least one
  signed-in device; `devicesRegistered` is always ≥ it, because people have two
  phones.
- **`appRequestShare` and every `adoptionTrend.appShare` are nullable, and `null`
...
**`hasApp` (`true`|`false`)**, `dormantMonths`, plus the §1.4 paging parameters.
Default `pageSize` 50, default sort `supplierCode` ascending.

**v2 adds three fields to the record and the row**, and they are the module's
subject now that the registry belongs to the factory's own console:
...
`?hasApp=false` is the list behind the dashboard's adoption percentage. A figure
nobody can turn into names is a figure nobody acts on.
...
`RequestTeaPacketsScreen` since its first release and there was no endpoint to decide
one, so a supplier could ask the factory for its own tea and the request went nowhere.
...
200 {
  "items": [{
    "id": "tea-1",
    "supplierId": "sup-3", "supplierCode": "5021 (MAKADURA)", "supplierName": "…",
    "packets": 4,
    ...
    "status": "pending",
    "createdAt": "2026-08-07T…", "channel": "app",
    "createdById": null, "createdByName": null,
    "decision": null,
    "recoveredOnMonthKey": null,
    "ageHours": 26
  }],
  "page": 0, "pageSize": 25, "total": 6
}
```

- `deliveryMethod` ∈ `factoryCollection | transportVehicle`. It is the storekeeper's
  working filter: a request going back on the collection vehicle has to be packed
  before the vehicle leaves.
- **`unitPrice` and `amount` are on the row, not derived by the console.** For a
  pending request they are today's catalogue price; for a decided one they are the
  price it was **decided at**, and must never be re-read from `client_config`
  afterwards. A catalogue edit that silently re-priced an answered request would put
  a figure on a supplier's account that nobody agreed to.
- `recoveredOnMonthKey` is the month whose published bill carried this on its
  `deductions.tea` line, or `null` while the factory is still owed for it. On the row
  rather than inferred from dates: a request approved after a month closed is
  recovered on the next one, and the difference is what `teaPacketsOutstanding`
  counts (§18).
...
Refusals, in the order the server must check them:

| Code | Status | When |
| --- | --- | --- |
| `feature-disabled` | 403 | `enableTeaPackets` is off (AC-07) |
| `note-required` | 422 | Under 10 characters, **on both verbs** (AC-06) |
| `already-decided` | 409 | Two clerks on one inbox. Include `decidedByName` in `details` |
| `four-eyes-violation` | 409 | The decider is `createdById` (BR-501). **Checked before the policy**, because who may decide does not depend on how many packets were asked for |
| `tea-packets-refused` | 409 | **Approval only.** The request is outside the factory's stock policy. `details` carries `problems` (`no-packets` · `not-whole` · `over-max`) and `maxPacketsPerRequest` |

**`tea-packets-refused` must not apply to a rejection**, and that asymmetry is
load-bearing: refusing both would trap the row in the queue for ever, with the office
unable either to issue the tea or to tell the supplier why. Same reasoning as BR-310
...
`docs/banners.md`) all shipped in v1 and nothing implemented them, so a factory that
turned the switch on got nothing.

Modelled on §16's articles: the same per-language `PUT`, the same three lifecycle
verbs, the same server-resolved preview. What a banner has that an article does not is
a **live window** and an **action**.

### 16a.1 `GET /admin/banners`

Query: `status`, `window` (`scheduled|live|expired`), `q`, plus §1.4 paging.
...
200 {
  "items": [{
    ...
    "hasImage": false,
    "missingLanguages": ["ta"], "staleLanguages": [],
    "updatedAt": "2026-08-07T…", "updatedByName": "Tharindu Silva"
  }],
...
- **A `window` filter implies `status: published`.** A draft is unfinished work, not
  ...
  office's "coming up" list that nobody has agreed to send.
- `title` is the **fallback** language's, as `NewsListItem.title` is: the office needs
  one column it can scan, and a row whose text changed with the selected tab would
  make the list unreadable while translating.

### 16a.2 `GET /admin/banners/{id}`

```json
200 {
  "id": "ban-1",
  "translations": {
    "en": { "lang": "en", "title": "…", "body": "…", "buttonLabel": "Request manure",
            "updatedAt": "…", "updatedByName": "…" }
  },
  "imageUrl": null, "imageAspectRatio": null,
  "action": { "type": "screen", "path": "manure" },
  "startsAt": "…", "endsAt": null,
  "status": "published", "publishedAt": "…", "publishedByName": "…",
...
- **`body` may be empty and `buttonLabel` may not.** "Written" for a banner means a
  ...
  `staleBannerTranslations` in `@tfd/domain` are the shared implementations.
- **`action` is not localized.** One destination in three languages; a banner whose
...
| `action` | Rule |
| --- | --- |
| `{type: 'screen', path}` | Must match `^[a-z][a-z0-9-]*(/[a-zA-Z0-9-]+)*$`. No `.`, so `..` cannot appear; no scheme, host or query string. A leading `/` is stripped |
| `{type: 'url', url}` | `https:`, `tel:`, `mailto:` only, parsed with `new URL` so malformed input is refused rather than normalised |

**`teafactory://` is refused deliberately**, and refused *by name*: an in-app
destination goes through `type: 'screen'`, where the path is matched against routes
that exist rather than being smuggled in as a URL. It is the mistake an editor makes on
purpose, and the answer is "use an in-app screen", not "unsupported scheme". The
console renders the specific rule from `details.problemKey`.
...
**A translation gap may be published over; a refused action may not.** The asymmetry is
the section's one real judgement and it follows from what the app does with each: a
missing language falls back to English and the supplier reads *something*, which AC-08
explicitly permits. A refused action has nothing to fall back to.

**The publish audit entry records the languages that will fall back**, and the window
state at the moment of publishing. *"Who decided a Tamil supplier could read this in
English"* is the question AC-08 turns into an argument six months later.

### 16a.5 What the app fetches
...
- [ ] **Call `POST /admin/notifications/trigger` on publish.** The endpoint is §17's and
      ...
      is a supplier who is never told their account is ready
